...
def absensiWajah():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            id, akurasi = faceRecognizer.predict(abuabu[y:y+h,x:x+w])
            print("debug =" + str(akurasi))
            if (akurasi > 85 ):
                id = names[0]
            else :
                id = "Tidak Diketahui"
            cv2.putText(frame, str(id), (x + 5, y - 5), font, 1, (255, 255, 255), 2)
            cv2.putText(frame, str(round(akurasi, 2)) + "%", (x + 5, y + h + 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
